...
jQuery(document).ready(function(){
	...
	///// SHOW/HIDE NOTIFICATION MESSAGE 消息页面展示/////
	...
	jQuery(document).click(function(event) {
		...
		var ab = jQuery('#messages .msglist a');
		...
		if(!jQuery(event.target).is('.noticontent') && nb.is(':visible')) {
			nb.remove();
			var id = ab.attr('id');
			var url = ab.attr('tilte');
			var name = ab.attr('name');
			//fngetBusinessMenu(url,id,name);
			jQuery('.notification').removeClass('active');
		}
	});
