...
jQuery(function(){
    var jQuerylist = jQuery("#thelist");
    var  uploader ;// 实例化
    uploader = WebUploader.create({
        auto:false, //是否自动上传
        pick: {
            id: '#attach',
            name:"file",  //这个地方 name 没什么用，如果想自定义file的name属性，还是要和fileVal 配合使用。
            label: '点击选择文件',
    ...
    uploader.on( "fileQueued", function( file ) {
        ...
        jQuerylist.append( "<div id='"+  file.id + "' class='item'>" +
    ...
    uploader.on( "uploadSuccess", function( file ) {
        //这里可以得到后台返回的数据
        jQuery( "#"+file.id ).find("p.state").text("已上传");
    });
